...
Call `agent-id current --json` to inspect the complete current assignment. The command uses `AGENT_ID_SESSION_ID` and never registers a missing identity. Call `agent-id discover` directly only when you need to find other identities. Results are newest first and include available summaries, lifecycle states, and working directories.

Lifecycle hooks publish `working`, `idle`, and `stopped` automatically. Use `agent-id annotate` to publish `waiting` or `blocked`, or to set or clear a summary when an explicit update is needed. Automatic summaries use completed agent turns; explicit updates remain authoritative.
